"""
Prayer of Dedication slide generation endpoints
"""
import os
import tempfile
import base64
...
from app.core.schemas import PrayerOfDedicationRequest
from pathlib import Path

router = APIRouter()


def create_prayer_of_dedication_slide(output_file, background_image=None, lead_pastor="Pastor"):
    """Create a prayer of dedication slide"""
    # Get the template path
    template_path = Path(__file__).parent.parent / "templates" / "prayer-of-dedication.pptx"
...
@router.post("/api/generate-prayer-of-dedication-slide")
async def generate_prayer_of_dedication_slide(request: PrayerOfDedicationRequest):
    """Generate a prayer of dedication slide with the lead pastor's name and background image"""
    try:
        # Get the template path
        template_path = Path(__file__).parent.parent / "templates" / "prayer-of-dedication.pptx"
        if not template_path.exists():
        ...
        # 1. First replace the {lead_pastor} text (shape at index 0 in this template)
        # The text structure is in shape 0 based on our examination
        if len(slide.shapes) > 0:
            text_shape = slide.shapes[0]
        ...
        with tempfile.NamedTemporaryFile(
            suffix='.pptx',
            delete=False,
            prefix='prayer_of_dedication_slide_'
        ) as tmp_file:
        ...
        return FileResponse(
            ...
            filename='prayer_of_dedication_slide.pptx',
            headers={
                'Content-Disposition': 'attachment; filename="prayer_of_dedication_slide.pptx"'
            }
        )
        
    except Exception as e:
        print(f"Error generating prayer of dedication slide: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Error generating prayer of dedication slide: {str(e)}"
        )